...
const MagicSpoon = {
    ...
    Orderbook(Server, baseBuying, counterSelling, onUpdate) {
        ...
        const streamTradeClosed = Server.orderbook(baseBuying, counterSelling).stream({
            onmessage: (res) => {
                let updated = false;
        ...
        const fetchManyTrades = async () => {
            ...
            const RESOLUTION = 900000; // All timeframes based on 15-min intervals
            const MAX_LOAD_DEPTH = 20; // multiplied on 200 elements

            while (loadedDepth < MAX_LOAD_DEPTH && !this.fullHistoryLoaded) {
                let tradeResults;
                loadedDepth += 1;

                if (firstFetch) {
                    tradeResults = await Server.tradeAggregation(
                        baseBuying,
                        counterSelling,
                        START_TIME,
                        END_TIME,
                ...
                records = [...records, ...tradeResults.records];

                // Returns valid ohlc data with volume for charts
                result = records.map(trade => ({
                ...
                if (tradeResults.records.length === 0) {
